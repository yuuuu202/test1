/*
 * 面向4KB消息长度的高性能完整性校验算法 - AES+SM3混合方案（极限优化版v2.1）
 * 基于ARMv8.2平台硬件加速指令优化
 ...
 * 
 * 性能提升策略（v2.1极限版）：
 * - SM3压缩次数：从64次降到4次（16x减少！）
 * - 去除AES指令：纯XOR折叠（2-3x提升）
 * - 更激进循环展开：4/2路展开（1.3-1.5x提升）
 * - NEON并行8块：向量化XOR（1.5-2x提升）
 * - 完全展开转换：消除所有小循环（1.2x提升）
 * - 总体预期提升：15-30x吞吐率（vs原始实现），10-15x vs SHA256
 */
